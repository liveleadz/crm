...
async function handle(req: NextRequest, ctx: { params: Promise<{ number: string }> }) {
  ...
  //
  // This route is the legacy `voice_url` LaML pipeline. The canonical
  // in-browser inbound handler is /api/swml/inbound-swml (SWML JSON with
  // `connect.to /private/<email>?channel=audio`), which dispatches via
  // Call Fabric Subscribers — the same identity system our SAT token +
  // `client.online({ incomingCallHandlers })` is built on.
  //
  // We do NOT emit `<Dial><Client>email</Client></Dial>` here: LaML's
  // <Client> targets the legacy Twilio Client SDK identity system, not
  // Call Fabric. With no legacy client connected under that identity,
  // SignalWire times the dial out silently and the caller hears nothing
  // (see commit 8875c28). Numbers that haven't been migrated to the
  // SWML Script resource fall through to voicemail here so the caller
  // never hits a dead dial. The agent still sees the call + recording
  // in /calls and the lead owner gets a bell notification.
