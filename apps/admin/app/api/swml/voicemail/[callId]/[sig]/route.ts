...
export async function POST(
  ...
  // Update the "Missed call" notification rows already created by the
  // inbound-swml route. We don't insert a new row — bell shows one entry
  // per missed call whether or not a voicemail was left. The voicemail
  // outcome upgrades the title + body so it reads "Voicemail from …".
  if (callRow) {
    ...
    try {
      await supabase
        ...
        .update({
          ...
          body: `${callRow.from_number} → ${callRow.to_number} · recording available`,
        })
        .eq('brand_id', callRow.brand_id)
        .eq('kind', 'inbound_call')
        .contains('data', { call_id: callId });
    } catch (e) {
      console.error('[voicemail:notify-update]', (e as Error).message);
    }
  }
